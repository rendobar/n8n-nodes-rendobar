...
export class Rendobar implements INodeType {
	description: INodeTypeDescription = {
		...
		description:
			'Run FFmpeg commands, compress and caption videos, and generate, edit or upscale images',
		defaults: { name: 'Rendobar' },
		// n8n's registry builds the listing shown before install from this
		// description, not from Rendobar.node.json, and the nodes panel search
		// matches `codex.alias`. With the codex only in the JSON file, the registry
		// stored `codex: {}` and a search for "ffmpeg" never found an uninstalled
		// Rendobar. The JSON file stays for loaders that read it, and
		// test/verification.test.js keeps the two identical.
		codex: {
