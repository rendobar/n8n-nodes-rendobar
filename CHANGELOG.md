...
- Every parameter hint is gone. Four fields carried one, and the verification
  ...
  field. **Input Binary Field** carried the same sentence twice, as a hint and as
  a description. The other three were caveats rather than help (the Wait node's
  HTTP Method and Limit Wait Time under **Callback URL**, key reuse under
  **Idempotency Key**, offset paging under **Return All**), and the README covers
  all three at more length than a hint has room for.
- Seven parameter descriptions are back to a sentence or two. **Idempotency Key**
  was 490 characters and **Wait for Completion** 369, both rendering as a block of
  grey text under a field inside a collection. The two **Output** selectors also
  stopped quoting how many fields a raw job or file carries, which describes
  Rendobar rather than the choice being made. A test caps a description at 220
  characters.
- Comments through the node, the credential and the test suite are shorter. The
  facts, traps and upstream bug references are unchanged; the prose around them
  is gone, along with about a third of the comment lines.
...
- The two parameter notices that fired before a job type was chosen ("Choose a
  job type above and its parameters load here"). The mapper draws its own empty
  state, so the line only repeated the panel back. The notices that remain each
  say something the panel cannot work out: that a job type's parameters have no
  form and need **Using JSON**, that `ffmpeg` and `ffprobe` name their input files
  in the command, and that a job type reads no input files at all.
- The "Give it its files in 'Inputs (JSON)'" half of the no-parameters notice.
  Input media has had its own mapped form since 0.4.0, so it pointed at the wrong
  field.
...
- **A single-buffer upload was copied for nothing**, a `Buffer.concat` on a one-element array, at that point up to 100 MB.
