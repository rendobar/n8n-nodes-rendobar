...
- **Create**: submit a job.
  ...
  - A parameter the job type gives a default for arrives filled in with it, and one it does not gives you an empty box. The exception is a number, which n8n cannot draw empty: it puts a `0` in the moment the box appears. Those wait under **Add parameter to send** instead, so nothing is sent for one until you ask for it. What the form shows is what gets sent, `0` included.
...
- **Get**: read the credit balance, the plan limits and the spend so far this period.
  ...
  - **There is no Usage operation.** `GET /billing/usage` carries no balance, so it does not answer what the balance events raise. It returns a per-job-type map and one row per date and job type, which is a chart rather than something a workflow branches on, and `usage.currentPeriodSpend` above is the spend figure to act on. **Custom API Call** reaches it if you want the breakdown.
...
- **Get Many**: the folders and files under **Folder** in a connected bucket, folders first. Each item has `type`, `path` and a `uri` such as `storage://prod-media/raw/clip.mp4` that a Job › Create input accepts. Files also carry `size` and `lastModified`.
...
Starts a workflow when a Rendobar media job completes, stops or is cancelled, or when the account balance runs low. Select the events to listen for: **Job Created**, **Job Started**, **Job Completed**, **Job Failed**, **Job Cancelled**, **Job Delivery Succeeded**, **Job Delivery Failed**, **Job Deliveries Settled**, **Balance Low** and **Balance Depleted**. On activation the node registers its webhook URL with Rendobar and removes it on deactivation.
...
result, and reads the answer out of `data`. None of it is configured in the node in
advance.
...
Rendobar can also sign a callback with an HMAC in `X-Rendobar-Signature`, which this node does not offer. The call lands on n8n's own resume endpoint rather than on a node, so nothing in the workflow can check a signature. Header Auth is the equivalent n8n can enforce, and step 3 above means the payload never has to be trusted.
...
- **A key you set in Idempotency Key is left exactly as you wrote it.** It states which submissions are the same submission, and only its author knows what changing it would mean, so a spent one is reported rather than quietly varied. If you want deliberate retries under your own key, make it differ between attempts by ending it in `{{ $runIndex }}`.
...
- **The Job list is empty.** It shows your recent jobs from `GET /jobs`. A brand new account has none yet, so switch the locator to **By ID** or **By URL**.
